...
class DataBase:
    ...
    @staticmethod
    def _update(logs: dict, mic_logs: dict):  # saves 2 dicts to settings
        settings.LOGS_GUILD_LIST, settings.MICROPHONE_GUILD_LIST = logs, mic_logs

    def get_number(self, name: str):
        self.c.execute('''SELECT number
                        FROM statistic
                        WHERE category = (?)
                        ''', (name,))
        number = self.c.fetchall()[0][0]
        return number

    def get_statistic_id(self):
        self.c.execute('''SELECT statistic_channel_id
                        FROM id_table
                        ''')
        guild = self.c.fetchall()
        return guild
